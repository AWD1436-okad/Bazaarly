...
export type CurrencyCode = (typeof CURRENCY_DATA)[number]["code"];
...
const countrySearchTerms: Partial<Record<CurrencyCode, string[]>> = {
  AED: ["United Arab Emirates", "UAE", "Dubai", "Abu Dhabi"],
  AUD: ["Australia"],
  GBP: ["United Kingdom", "Britain", "England", "Scotland", "Wales"],
  PKR: ["Pakistan"],
  SAR: ["Saudi Arabia", "Saudi", "KSA"],
  USD: ["United States", "USA", "America"],
};

export function getCurrencySearchTerms(currency: CurrencyData) {
  return [currency.code, currency.name, currency.symbol, ...(countrySearchTerms[currency.code] ?? [])];
}
