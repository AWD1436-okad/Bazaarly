...
async function main() {
  ...
  for (const product of PRODUCT_CATALOG) {
    ...
    await prisma.marketProductState.create({
      data: {
  ...
  const botWalletUser = await prisma.user.create({
    data: {
      username: "bot_market",
  ...
  const botWalletShop = await prisma.shop.create({
    data: {
  ...
  for (const bot of INITIAL_BOTS) {
    await prisma.botCustomer.create({
      data: {
